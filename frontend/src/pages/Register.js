            ...
            <Fragment>
                <div className="mx-auto px-4 md:px-10 lg:px-20 xl:px-48 mt-4">
                    ...
                    <div className="flex flex-col items-center mt-12">
                        ...
                        <p className="mt-5 w-full sm:w-2/4 md:w-2/3 lg:w-1/3 text-center">By registering, I agree to the Hotels <button className="text-blue-500">Terms and Conditions </button> and <button className="text-blue-500">Privacy Statement</button>.</p>
                        <button onClick={signupHandler} type="button" className=" mt-12 p-3 w-2/3 sm:w-2/4 md:w-1/3 lg:w-1/5 text-white rounded-3xl hover:bg-blue-900 bg-blue-700 disabled:bg-blue-500" disabled={email.length < 1 || password.length < 8 || password !== confirmPassword || name.length < 1 ? true : false || loading}>Register</button>
                        <p className="mt-4">Already have an account? <Link to="/login" className="text-blue-700" >Login</Link></p>
                    </div>
                </div>
            </Fragment>
        </Fragment>
